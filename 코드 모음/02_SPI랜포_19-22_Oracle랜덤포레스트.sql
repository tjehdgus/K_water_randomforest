-- SPI 랜덤 포레스트 (19-22년 데이터)
-- 관련 CSV: 랜덤포레스트테이블.csv, 가뭄수치넣은가뭄데이터.csv, 계절넣은누적강수데이터.csv

-- 테이블 생성
drop table w;
create table w
as
select rownum as id, s.가뭄, d.누적강수량, d.계절,
w.가조시간
...
-- drou 테이블에 가뭄 컬럼 추가 및 SPI6 기준 분류
alter table drou add 가뭄 varchar2(20);
update drou
    set 가뭄 = '심한습윤'
    where spi6 >= 1.50;
update drou
